...
def request_leave():
    try:
        ...
        employee = User.query.get_or_404(employee_id)
        if leave_type.title() not in ['Sick', 'Casual', 'LOP', 'Lop']:
            return jsonify({'error': 'Invalid leave type'}), 400

        # Ensure a LeaveBalance exists and enforce limits for Sick/Casual
        balance = LeaveBalance.query.filter_by(employee_id=employee.id).first()
        if not balance:
            balance = LeaveBalance(employee_id=employee.id)
            db.session.add(balance)
        ...
        lt = leave_type.title()
        if lt == 'Sick' and days > balance.sick_leave:
            return jsonify({'error': f'Requested {days} Sick days exceeds remaining {balance.sick_leave}'}), 400
        if lt == 'Casual' and days > balance.casual_leave:
            return jsonify({'error': f'Requested {days} Casual days exceeds remaining {balance.casual_leave}'}), 400
