...
def move_candidate_in_pipeline(candidate_id):
    ...
    try:
        ...
        # Send notification to candidate when moved to specific stages
        notification_message = None
        if new_stage.name.lower() == 'hired':
            notification_message = f"🎉 Congratulations! You have been hired for the position of {job.title}. Welcome to the team!"
        elif new_stage.name.lower() == 'offer':
            notification_message = f"📧 Great news! You've reached the Offer stage for {job.title}. An offer letter will be sent soon."
        elif 'interview' in new_stage.name.lower():
            notification_message = f"📅 You've been moved to {new_stage.name} stage for {job.title}. Please check your interviews."
        elif new_stage.name.lower() == 'technical test':
            notification_message = f"📝 You've been assigned to the Technical Test stage for {job.title}. Check your dashboard for test details."
...
def assign_interviewer():
    ...
    try:
        ...
        # Send notification to the assigned interviewer (HO, BDA, or Manager)
        interview_type_display = {
            ...
            'hr': 'HR Interview'
        }.get(data.get('interview_type', 'technical'), 'Interview')
        
        notification_message = f"📅 You have been assigned to conduct a {interview_type_display} for {candidate.name} (Position: {job.title})."
        if scheduled_at:
            notification_message += f" Scheduled at: {scheduled_at.strftime('%b %d, %Y %I:%M %p')}"
        ...
        # Send notification to the candidate as well
        candidate_notification_message = f"📅 An interview has been scheduled for {interview_type_display} with {interviewer.name} (Position: {job.title})."
        if scheduled_at:
            candidate_notification_message += f" Scheduled at: {scheduled_at.strftime('%b %d, %Y %I:%M %p')}"
        else:
            candidate_notification_message += " The interviewer will contact you soon to schedule a time."
        
        candidate_notification = Notification(
            recipient_id=candidate_id,
            message=candidate_notification_message,
            read=False
        )
...
def get_interview_assignments():
    ...
    try:
        ...
        if upcoming_only:
            # Include both scheduled interviews in the future AND unscheduled interviews (pending scheduling)
            from sqlalchemy import or_
            query = query.filter(
                or_(
                    InterviewerAssignment.scheduled_at > datetime.utcnow(),
                    InterviewerAssignment.scheduled_at == None
                ),
