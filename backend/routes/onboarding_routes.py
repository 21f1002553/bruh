...
def verify_document(doc_id):
    ...
    Only HR/Admin can verify or unverify.
    """
    current_user_id = get_jwt_identity()
    ...
    if (current_user.role_id == role.id) and ((role.name == "admin") or (role.name == "manager") or (role.name == "hr")):
        try:
            ...
            # Get verified status from request body (default to True for backward compatibility)
            verified_status = data.get('verified', True)
            ...
            if verified_status:
                ...
                message = f"onboarding {doc.filename} verified successfully"
                action = "verify"
            else:
                # Unverifying the document
                if not doc.verified:
                    return jsonify({'error': 'Document is not verified'}), 400
                doc.verified = False
                doc.verified_by = None
                doc.verified_at = None
                message = f"onboarding {doc.filename} unverified"
                action = "unverify"
            
            db.session.commit()
            notify(doc.employee_id, message)
            create_audit(current_user_id, action, "onboarding", doc.id)
            return jsonify({'message': message, 'document': doc.to_dict()}), 200
        except Exception as e:
            db.session.rollback()
            return jsonify({'error': str(e)}), 500
    else:
...
@onboarding_bp.route('/documents/<employee_id>/<doc_id>/download', methods=['GET'])
@jwt_required()
def download_onboarding_document(employee_id, doc_id):
    """
    Download an onboarding document.
    Only HR/Admin/Manager or the employee can download.
    """
    current_user_id = get_jwt_identity()
    try:
        current_user = User.query.get_or_404(current_user_id)
        ...
        # Get document
        doc = OnBoardingDocument.query.get_or_404(doc_id)
        if str(doc.employee_id) != str(employee_id):
            return jsonify({'error': 'Document not found for this employee'}), 404
