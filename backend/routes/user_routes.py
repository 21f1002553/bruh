...
def create_user():
    ...
    current_user = User.query.get_or_404(current_user_id)
    role = Role.query.filter_by(id=current_user.role_id).first()
    if (current_user.role_id == role.id) and (role.name != "hr"):
...
def get_employees():
    try:
        ...
        # Allow only HR to view employees
        if not role or role.name.lower() != "hr":
            return jsonify({'error': 'Only HR can view employees'}), 403

        # All non-candidate users as employees
        users = User.query.all()
        employees = [
            u.to_dict()
            for u in users
            if u.role and u.role.name.lower() != "candidate"
        ]
