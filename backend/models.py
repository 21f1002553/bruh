...
class InterviewerAssignment(db.Model):
    ...
    def to_dict(self):
        # Get the scorecard if it exists
        scorecard = None
        if self.scorecards:
            latest_scorecard = self.scorecards[0]  # Assuming one scorecard per assignment
            scorecard = latest_scorecard.to_dict()
...
class AITestAssessment(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'candidate_id': self.candidate_id,
            'score': self.score,
            'score_percentage': self.score,  # Alias for frontend
            'feedback': self.feedback,
            'strengths': self.strengths,
            'weaknesses': self.weaknesses,
            'recommendation': self.recommendation,
