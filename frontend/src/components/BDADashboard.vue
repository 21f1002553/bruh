    ...
    <div class="bda-dashboard">
      ...
      <div class="dashboard-header">
        ...
        <div class="quick-actions">
          ...
          <button class="quick-action-btn" @click="router.push('/bda/eod')">
            ...
            <div v-else class="expenses-list">
              <div v-for="expense in recentExpenses" :key="expense.id" class="expense-item">
                ...
                <div class="expense-info">
                  <h4>{{ expense.items[0].category || 'General Expense' }}</h4>
                  <p>{{ formatDate(expense.items[0].expense_date) }}</p>
                </div>
        ...
        <div class="dashboard-card eod-card">
          ...
          <div class="card-body">
            ...
            <div v-else class="eod-list">
              <div v-for="report in recentEODReports" :key="report.id" class="eod-item">
                <div class="eod-date">
        ...
        <div class="modal-container">
          ...
          <div class="modal-body">
            <form @submit.prevent="submitExpense">
              ...
              <div class="form-group">
                <label>Amount (₹)</label>
                <input v-model.number="expenseForm.amount" type="number" step="0.01" placeholder="Enter amount"
                  required />
              </div>
              <div class="form-group">
                <label>Description</label>
                <textarea v-model="expenseForm.description" placeholder="Describe the expense..." rows="3"
                  required></textarea>
              </div>
              <div class="form-group">
                <label>Receipt (Optional)</label>
                <input type="file" @change="handleReceiptUpload" accept=".pdf,.jpg,.jpeg,.png" />
              </div>
              <div class="modal-footer">
                <button type="button" class="btn-cancel" @click="showExpenseModal = false">
                  Cancel
                </button>
                <button type="submit" class="btn-submit" @click="submitExpense" :disabled="submittingExpense">
        ...
        <div class="modal-container">
          ...
          <div class="modal-body">
            <form @submit.prevent="submitEOD">
              <div class="form-group">
                <label>Date</label>
                <input v-model="eodForm.date" type="date" :max="getCurrentDate()" required />
              </div>
              <div class="form-group">
                <label>Client Visits</label>
                <textarea v-model="eodForm.client_visits" placeholder="List client visits and outcomes..." rows="3"
                  required></textarea>
              </div>
              <div class="form-group">
                <label>Leads Generated</label>
                <input v-model.number="eodForm.leads_generated" type="number" min="0" placeholder="Number of leads"
                  required />
              </div>
              <div class="form-group">
                <label>Follow-ups Completed</label>
                <input v-model.number="eodForm.followups_completed" type="number" min="0"
                  placeholder="Number of follow-ups" required />
              </div>
              <div class="form-group">
                <label>Challenges Faced</label>
                <textarea v-model="eodForm.challenges" placeholder="Any challenges or issues..." rows="2"></textarea>
              </div>
              <div class="form-group">
                <label>Next Day Plan</label>
                <textarea v-model="eodForm.next_day_plan" placeholder="Plan for tomorrow..." rows="2"></textarea>
              </div>
              <div class="modal-footer">
            ...
            <form @submit.prevent="submitEvaluation">
              <div class="evaluation-section">
                ...
                <div class="form-group">
                  ...
                  <input v-model.number="evaluationForm.cultural_fit_score" type="number" min="0" max="100" required />
                </div>
                <div class="form-group">
              ...
              <div class="form-group">
                ...
                <textarea v-model="evaluationForm.strengths" placeholder="Key strengths observed..." rows="3"
                  required></textarea>
              </div>

              <div class="form-group">
                <label>Weaknesses</label>
                <textarea v-model="evaluationForm.weaknesses" placeholder="Areas for improvement..." rows="3"
                  required></textarea>
              </div>

              <div class="form-group">
                <label>Additional Feedback</label>
                <textarea v-model="evaluationForm.feedback_notes" placeholder="Any additional comments..."
                  rows="3"></textarea>
              </div>
...
watch([() => leaveForm.value.leave_type, () => leaveBalance.value?.sick_leave, () => leaveBalance.value?.casual_leave], () => {
  const max = maxAllowedDays.value
  if (max !== null && leaveForm.value.days > max) {
...
async function loadBDAData() {
  try {
    ...
    console.error('Error loading BDA data:', error)
  }
}
...
async function loadUpcomingInterviews() {
  ...
  try {
    const token = getAuthToken()
    // Get interviews where BDA is the interviewer and type is behavioral
