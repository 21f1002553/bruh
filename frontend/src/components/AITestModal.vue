    ...
    <div class="modal-container ai-test-modal" @click.stop>
      ...
      <div class="modal-header">
        <h3>
          ...
          AI Technical Test{{ selectedCandidateInfo ? ' - ' + selectedCandidateInfo.name : '' }}
        </h3>
      ...
      <div class="modal-body">
        <form @submit.prevent="generateTest" class="test-form">
          <!-- Candidate Selection (if multiple candidates available) -->
          <div v-if="candidates && candidates.length > 0" class="form-group">
            <label class="form-label required">
              <i class="fas fa-user"></i>
              Select Candidate
            </label>
            <select v-model="testData.selected_candidate_id" required class="form-select candidate-select">
              <option value="">-- Choose a candidate --</option>
              <option v-for="cand in candidates" :key="cand.id" :value="cand.id">
                {{ cand.name }} - {{ cand.email }} ({{ cand.job_title || 'N/A' }})
              </option>
            </select>
          </div>

          <!-- Candidate Info (if pre-selected) -->
          <div v-else-if="candidate" class="candidate-info-section">
            <div class="info-row">
              <div class="info-item">
...
const isFormValid = computed(() => {
  const hasCandidate = props.candidate || testData.value.selected_candidate_id
  return hasCandidate && testData.value.duration && testData.value.deadline
})

const selectedCandidateInfo = computed(() => {
  if (props.candidate) return props.candidate
  if (testData.value.selected_candidate_id && props.candidates) {
    return props.candidates.find(c => c.id === testData.value.selected_candidate_id)
  }
  return null
})
...
async function generateTest() {
  ...
  try {
    ...
    const candidateId = props.candidate?.id || testData.value.selected_candidate_id
