    ...
    <div class="ho-dashboard">
      ...
      <div class="dashboard-header">
        ...
        <div class="quick-actions">
          ...
          <button class="quick-action-btn" @click="navigate('task-management')">
        ...
        <div class="dashboard-card vacancies-card full-width">
          ...
          <div class="card-body">
            ...
            <div v-else class="vacancies-grid">
              <div v-for="vacancy in recentVacancies" :key="vacancy.id" class="vacancy-item">
        ...
        <div class="dashboard-card interviews-card">
          ...
          <div class="card-body">
            ...
            <div v-else class="interviews-list">
              <div v-for="interview in technicalInterviews" :key="interview.id" class="interview-item">
                ...
                <div class="interview-actions">
                  <button v-if="interview.meeting_link" class="btn-join" @click="openMeeting(interview.meeting_link)">
                    <i class="fas fa-video"></i> Join
                  </button>
                  <button class="btn-evaluate" @click="openEvaluationModal(interview)">
                    <i class="fas fa-clipboard-check"></i>
                    {{ interview.scorecard ? 'Edit Evaluation' : 'Evaluate' }}
                  </button>
                </div>
              </div>
            </div>
        ...
        <div class="dashboard-card tasks-card">
          ...
          <div class="card-body">
            ...
            <div v-else class="tasks-list">
              <div v-for="task in assignedTasks" :key="task.id" class="task-item">
        ...
        <div class="dashboard-card leave-card">
          ...
          <div class="card-body">
            ...
            <div v-if="recentLeaves.length > 0" class="recent-leaves">
              ...
              <div v-for="leave in recentLeaves.slice(0, 3)" :key="leave.id" class="leave-item">
                <div class="leave-dates">
                  {{ formatDate(leave.date) }} - {{ formatDate(new Date(Date.parse(leave.date) + leave.days * 24 * 60 *
                    60 * 1000)) }}
                </div>
          ...
          <div class="modal-body">
            <form @submit.prevent="submitVacancy">
              <div class="form-group">
                ...
                <input v-model="vacancyForm.title" type="text" placeholder="e.g., Software Engineer" required />
              </div>
              <div class="form-row">
                <div class="form-group">
                  <label>Location *</label>
                  <input v-model="vacancyForm.location" type="text" placeholder="e.g., Mumbai" required />
                </div>
                <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input v-model="vacancyForm.school" type="text"
                  :placeholder="vacancyForm.org_type === 'school' ? 'School Name' : 'Company Name'" required />
              </div>
              <div class="form-row">
                <div class="form-group">
                  <label>Level</label>
                ...
                <div class="form-group">
                  ...
                  <input v-model.number="vacancyForm.count" type="number" min="1" required />
                </div>
              </div>
              <div class="form-group">
                <label>Description</label>
                <textarea v-model="vacancyForm.description" placeholder="Job description..." rows="3"></textarea>
              </div>
              <div class="form-group">
                <label>Requirements</label>
                <textarea v-model="vacancyForm.requirements" placeholder="Job requirements..." rows="3"></textarea>
              </div>
              <div class="form-group">
                <label>Salary Range</label>
                <input v-model="vacancyForm.salary_range" type="text" placeholder="e.g., 5-8 LPA" />
              </div>
              <div class="form-group">
                <label>
                  <input v-model="vacancyForm.notify_hr" type="checkbox" />
          ...
          <div class="modal-body">
            <form @submit.prevent="submitTask">
              ...
              <div class="form-group">
                ...
                <input v-model="taskForm.title" type="text" placeholder="e.g., Complete Python Assignment" required />
              </div>
              <div class="form-group">
                <label>Description *</label>
                <textarea v-model="taskForm.description" placeholder="Detailed task description..." rows="4"
                  required></textarea>
              </div>
              <div class="form-row">
                <div class="form-group">
                  <label>Deadline *</label>
                  <input v-model="taskForm.deadline" type="datetime-local" :min="getCurrentDateTime()" required />
                </div>
                <div class="form-group">
            ...
            <form @submit.prevent="submitLeave">
              ...
              <div class="form-row">
                <div class="form-group">
                  ...
                  <input v-model="leaveForm.start_date" type="date" :min="getCurrentDate()" required />
                </div>
                <div class="form-group">
                  <label>Number of Days</label>
                  <input v-model.number="leaveForm.days" type="number" min="0.5" step="0.5"
                    :max="maxAllowedDays !== null ? maxAllowedDays : null" placeholder="e.g., 1, 1.5, 2" required />
              ...
              <div class="form-group">
                <label>Reason</label>
                <textarea v-model="leaveForm.reason" placeholder="Reason for leave..." rows="3" required></textarea>
              </div>
              <div class="modal-footer">
                <button type="button" class="btn-cancel" @click="showLeaveModal = false">
                  Cancel
                </button>
                <button type="submit" class="btn-submit" :disabled="submittingLeave || !canSubmitLeave">
            ...
            <form @submit.prevent="submitEvaluation">
              <div class="evaluation-section">
                ...
                <div class="form-group">
                  ...
                  <input v-model.number="evaluationForm.technical_score" type="number" min="0" max="100" required />
                </div>
                <div class="form-group">
                  <label>Problem Solving Score (0-100) *</label>
                  <input v-model.number="evaluationForm.problem_solving_score" type="number" min="0" max="100"
                    required />
                </div>
                <div class="form-group">
                  <label>Communication Score (0-100) *</label>
                  <input v-model.number="evaluationForm.communication_score" type="number" min="0" max="100" required />
                </div>
              </div>

              <div class="form-group">
                <label>Technical Strengths *</label>
                <textarea v-model="evaluationForm.strengths" placeholder="Technical skills and strengths observed..."
                  rows="3" required></textarea>
              </div>

              <div class="form-group">
                <label>Areas for Improvement *</label>
                <textarea v-model="evaluationForm.weaknesses" placeholder="Technical gaps or areas to improve..."
                  rows="3" required></textarea>
              </div>

              <div class="form-group">
                <label>Additional Technical Feedback</label>
                <textarea v-model="evaluationForm.feedback_notes" placeholder="Any additional technical observations..."
                  rows="3"></textarea>
              </div>
...
// Computed property for maximum allowed days based on leave type
const maxAllowedDays = computed(() => {
  const t = leaveForm.value.leave_type
  if (!t || !leaveBalance.value) return null

  if (t === 'Sick') return Number(leaveBalance.value.sick_leave ?? 0)
  if (t === 'Casual') return Number(leaveBalance.value.casual_leave ?? 0)
  // LOP has no limit
  return null
})

// Computed property to check if leave can be submitted
const canSubmitLeave = computed(() => {
  const max = maxAllowedDays.value
  if (max !== null && leaveForm.value.days > max) {
    return false
  }
  return true
})

// Watch for leave type or balance changes and clamp days value
watch([
  () => leaveForm.value.leave_type,
  () => leaveBalance.value?.sick_leave,
  () => leaveBalance.value?.casual_leave
], () => {
  const max = maxAllowedDays.value
  if (max !== null && leaveForm.value.days > max) {
    leaveForm.value.days = max
  }
})
...
function navigate(page) {
  const routes = {
    ...
    'task-management': '/ho/task-management'
  }
  if (routes[page]) {
...
function openEvaluationModal(interview) {
  ...
  // Pre-fill form if editing existing scorecard
  if (interview.scorecard) {
    evaluationForm.value = {
      technical_score: interview.scorecard.technical_score || 0,
      problem_solving_score: interview.scorecard.problem_solving_score || 0,
      communication_score: interview.scorecard.communication_score || 0,
...
async function loadRecentVacancies() {
  ...
  try {
    ...
    recentVacancies.value = response.data || []
...
async function loadTechnicalInterviews() {
  ...
  try {
    ...
    // Map the assignments to display format
    technicalInterviews.value = (response.data.interview_assignments || []).map(interview => ({
      ...interview,
...
async function notifyHR(vacancy) {
  try {
    ...
    for (const hr of hrUsers) {
      await axios.post('/api/notifications/sends', {
        recipient_id: hr.id,
