        ...
        <div v-if="scorecard" class="scorecard-content">
          ...
          <div class="interview-details">
            <div class="detail-grid">
              <div class="detail-item" v-if="scorecard.job_title">
                <label>Position:</label>
                <span>{{ scorecard.job_title }}</span>
              </div>
              ...
              <div class="detail-item">
                <label>Date:</label>
                <span>{{ formatDate(scorecard.submitted_at) }}</span>
              </div>
              <div class="detail-item" v-if="scorecard.is_final !== null && scorecard.is_final !== undefined">
                <label>Status:</label>
                <span>{{ scorecard.is_final ? 'Final' : 'Draft' }}</span>
              </div>
            </div>
          </div>
          ...
          <!-- Comments -->
          <div v-if="scorecard.feedback_notes" class="comments-section">
            <h4>Additional Comments</h4>
            <div class="comments-box">
              <p>{{ scorecard.feedback_notes }}</p>
            </div>
          </div>
...
const individualScores = computed(() => {
  ...
  // Handle both formats: individual_scores object or flat properties
  if (props.scorecard.individual_scores) {
    return Object.entries(props.scorecard.individual_scores).map(([criteria, data]) => ({
      ...
      value: typeof data === 'object' ? (data.score || 0) : data,
      notes: typeof data === 'object' ? data.notes : null
    }))
  }
  
  // Handle flat API response format
  const scores = []
  if (props.scorecard.technical_score !== null && props.scorecard.technical_score !== undefined) {
    scores.push({
      criteria: 'Technical',
      value: Number(props.scorecard.technical_score),
      notes: null
    })
  }
  if (props.scorecard.communication_score !== null && props.scorecard.communication_score !== undefined) {
    scores.push({
      criteria: 'Communication',
      value: Number(props.scorecard.communication_score),
      notes: null
    })
  }
  if (props.scorecard.problem_solving_score !== null && props.scorecard.problem_solving_score !== undefined) {
    scores.push({
      criteria: 'Problem Solving',
      value: Number(props.scorecard.problem_solving_score),
      notes: null
    })
  }
  if (props.scorecard.cultural_fit_score !== null && props.scorecard.cultural_fit_score !== undefined) {
    scores.push({
      criteria: 'Cultural Fit',
      value: Number(props.scorecard.cultural_fit_score),
      notes: null
    })
  }
