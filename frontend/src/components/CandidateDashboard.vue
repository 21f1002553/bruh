        ...
        <div class="dashboard-grid">
          ...
          <div class="dashboard-card notifications-card">
            ...
            <div v-else-if="notifications.length > 0" class="notifications-list">
              ...
              >
                <div class="notification-icon" :class="getNotificationIcon(notification.message)">
                  <i :class="getNotificationIconClass(notification.message)"></i>
                </div>
  ...
  <div v-else class="no-interviews">
    ...
    <p>No upcoming interviews</p>
      ...
      <div class="dashboard-card tasks-summary">
        ...
        <!-- Tasks Summary -->
        <div v-else-if="todayTasks && todayTasks.length > 0" class="tasks-summary-content">
          ...
          <div class="recent-tasks-list">
            ...
            <div v-for="task in recentTasks" :key="task.id" class="mini-task-item">
              <div class="mini-task-info">
                ...
                <span class="mini-status-badge" :class="task.status">{{ formatTaskStatus(task.status) }}</span>
              </div>
              <div class="mini-task-meta">
                <span class="mini-deadline" :class="{ 'near': isDeadlineNear(task.deadline) }">
                  <i class="fas fa-clock"></i>
          ...
          <button @click="navigate('candidate-tasks')" class="view-all-btn">
...
const pendingTasksCount = computed(() => {
  return todayTasks.value.filter(t => t.status === 'pending').length;
});

const inProgressTasksCount = computed(() => {
  return todayTasks.value.filter(t => t.status === 'in_progress').length;
});

const completedTasksCount = computed(() => {
  return todayTasks.value.filter(t => t.status === 'completed' || t.status === 'reviewed').length;
});
...
function navigate(page) {
  const pageMap = {
    ...
    "candidate-tasks": "/candidate/tasks",
  };
  ...
  try {
    ...
    if (response.data) {
      ...
      notifications.value = response.data
        .filter(n => n.recipient_id === user_id.value)
...
function getNotificationIcon(message) {
  if (message.includes('🎉') || message.includes('hired') || message.includes('Hired')) {
    return 'success';
  } else if (message.includes('📧') || message.includes('Offer')) {
    return 'offer';
  } else if (message.includes('📅') || message.includes('interview')) {
    return 'interview';
  } else if (message.includes('📝') || message.includes('test') || message.includes('Test')) {
    return 'test';
  } else {
    return 'info';
  }
...
async function loadUpcomingInterviews() {
  try {
    const response = await axios.get(
      `/api/pipeline/interview-assignments`,
      {
        headers: { Authorization: `Bearer ${localStorage.getItem("access_token")}` },
        params: { 
          candidate_id: user_id.value,
          upcoming_only: true,
          limit: 10 
        },
      }
    ...
    if (response.data?.interview_assignments) {
      upcomingInterviews.value = response.data.interview_assignments.map((interview) => ({
        id: interview.id,
        jobTitle: interview.job_title || "Interview",
        company: interview.job?.school || "Company",
