          ...
          <div class="candidate-cards" v-if="!loading">
            <div v-for="candidate in getCandidatesInStage(stage.id)" :key="candidate.id" class="candidate-card"
              ...
              <div class="candidate-metrics">
                ...
                <!-- Show AI technical test score in Technical Test stage -->
                <div class="metric-item" v-if="stage.name === 'Technical Test' && candidate.test_score !== null && candidate.test_score !== undefined">
                  <span class="metric-label">AI Test Score:</span>
                  <span class="metric-value score" :class="getScoreClass(candidate.test_score)">
                    {{ candidate.test_score }}%
                  </span>
                </div>
                <!-- Show interview scorecard in all other stages if available -->
                <div class="metric-item" v-if="stage.name !== 'Technical Test' && candidate.scorecard">
                  <span class="metric-label">Interview Score:</span>
                  <span class="metric-value score" :class="getScoreClass(candidate.scorecard.overall_score * 20)">
                    {{ candidate.scorecard.overall_score?.toFixed(1) || 'N/A' }}/5
                  </span>
                </div>
                <div class="metric-item">
              ...
              <div v-if="isOfferStage(stage)" class="offer-section">
                ...
                <div v-if="candidate.offer_letter" class="offer-status">
                  ...
                  <!-- Move to Onboarding Button (only if offer is accepted) -->
                  <div v-if="candidate.offer_letter.acceptance_status === 'accepted'" class="move-to-onboarding-section">
                    <button @click="moveToOnboarding(candidate)" class="action-btn move-onboarding-btn"
              ...
              <div class="card-actions">
                <!-- AI Test Button (for Technical Test stage) -->
                <button v-if="stage.name === 'Technical Test'" @click="generateTechnicalTestForCandidate(candidate)" 
                  class="action-btn generate-test-btn">
                  <i class="fas fa-robot"></i>
                  AI Test
                </button>

                <!-- View Test Results Button (if test exists) -->
                <!-- <button v-if="candidate.ai_test_id" @click="openTestResults(candidate, candidate.ai_test_id)" 
                  class="action-btn view-test-btn">
...
function getInterviewersForStage(stage) {
  ...
  const stageInterviewerMap = {
    ...
    'Final Interview': [...HRinterviewers.value, ...BDAinterviewers.value]
  }
...
async function loadHoInterviewers() {
  try {
    const response = await axios.get('/api/users/role/ho', {
      headers: { Authorization: `Bearer ${getAuthToken()}` }
    })
...
async function loadPipelineCandidates() {
  try {
    ...
    console.log(candidatesInPipeline.value)
      ...
      await loadAITestScores()
      await loadScorecards()  // Load scorecards after interview assignments and test scores
...
async function assignInterviewer(candidateId, interviewer, stage) {
  try {
    ...
    const response = await axios.post('/api/pipeline/assign-interviewer', {
      ...
      duration_minutes: 60,
      assigned_by_id: userData.value.id,
...
async function generateTechnicalTestForCandidate(candidate) {
  try {
    // Load job details for this candidate
    const jobResponse = await axios.get(`/api/jobs/${candidate.job_id}`, {
      headers: { Authorization: `Bearer ${getAuthToken()}` }
    })
...
// Load AI Test scores for all candidates
async function loadAITestScores() {
  try {
    for (const candidate of candidatesInPipeline.value) {
      try {
        // Get AI test for this specific candidate
        const response = await axios.get(`/api/ai/ho/technical-tests`, {
          headers: { Authorization: `Bearer ${getAuthToken()}` }
        })

        if (response.data?.tests) {
          // Find test for this candidate
          const candidateTest = response.data.tests.find(t => t.candidate_id === candidate.id)
          
          if (candidateTest) {
            candidate.ai_test_id = candidateTest.id
            
            // If test has assessment, get the score
            if (candidateTest.assessment) {
              candidate.test_score = candidateTest.assessment.score_percentage || candidateTest.assessment.score
            }
          }
        }
      } catch (error) {
        console.error(`Failed to load AI test for candidate ${candidate.id}:`, error)
      }
    }
  } catch (error) {
    console.error('Failed to load AI test scores:', error)
  }
}
...
async function moveToOnboarding(candidate) {
  if (!confirm(`Move ${candidate.name} to Onboarding stage? This will start the document collection process.`)) {
    return
  }
  ...
  try {
    ...
    // Find the Onboarding stage
    const onboardingStage = pipelineStages.value.find(s => s.name.toLowerCase() === 'onboarding')
    
    if (!onboardingStage) {
      alert('Onboarding stage not found in pipeline. Please create an Onboarding stage first.')
      return
    }

    // Move candidate to Onboarding stage
    const response = await axios.put(`/api/pipeline/candidates/${candidate.id}/move`, {
      stage_id: onboardingStage.id,
      job_id: candidate.job_id,
      notes: 'Moved to onboarding after offer acceptance',
      moved_by_id: userData.value.id
    }, {
      headers: { Authorization: `Bearer ${getAuthToken()}` }
    })
    ...
    if (response.data) {
      alert(`${candidate.name} has been moved to Onboarding stage! They can now upload required documents.`)
      await refreshPipeline()
    }
  } catch (error) {
    console.error('Error moving to onboarding:', error)
    alert(error.response?.data?.error || 'Failed to move candidate to onboarding. Please try again.')
  } finally {
    processingCandidate.value = null
  }
}
    ...
    for (const candidate of candidatesInPipeline.value) {
      try {
        ...
        if (response.data?.scorecards && response.data.scorecards.length > 0) {
          // Find the current stage's interview type
          const currentStageInterviewType = checkInterviewType(candidate.stage_name)
          
          // Filter scorecards to match current stage's interview type
          let matchingScorecard = null
          
          for (const scorecard of response.data.scorecards) {
            if (scorecard.interview_assignment_id && candidate.interview_assignments) {
              // Find the matching interview assignment
              const matchingAssignment = candidate.interview_assignments.find(
                a => a.id === scorecard.interview_assignment_id
              )
              
              // Check if this scorecard matches the current stage
              if (matchingAssignment && matchingAssignment.interview_type === currentStageInterviewType) {
                matchingScorecard = scorecard
                break
              }
            }
          }
          
          // If no matching scorecard for current stage, skip
          if (!matchingScorecard) {
            candidate.scorecard = null
            continue
          }
          
          const scorecard = matchingScorecard
          
          // Get interview assignment details if available
          let interviewType = 'Technical Interview'
          let duration = 60
          
          if (scorecard.interview_assignment_id && candidate.interview_assignments) {
            const assignment = candidate.interview_assignments.find(
              a => a.id === scorecard.interview_assignment_id
            )
            if (assignment) {
          ...
          candidate.scorecard = {
            ...
            interviewer_name: scorecard.interviewer_name || 'Unknown',
            interview_date: scorecard.submitted_at,
            duration: duration,
            individual_scores: {
              'Technical Skills': { score: scorecard.technical_score || 0 },
              'Communication': { score: scorecard.communication_score || 0 },
              'Problem Solving': { score: scorecard.problem_solving_score || 0 },
              'Cultural Fit': { score: scorecard.cultural_fit_score || 0 }
            }
          }
        }
      } catch (error) {
...
.move-onboarding-btn:hover:not(:disabled) {
  transform: translateY(-2px);
  box-shadow: 0 4px 12px rgba(39, 174, 96, 0.4);
}

.move-onboarding-btn i {
  margin-right: 6px;
}
