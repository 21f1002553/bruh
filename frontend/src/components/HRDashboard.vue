      ...
      <div class="dashboard-grid">
        ...
        <div class="dashboard-card notifications-card">
          <div class="card-header">
            ...
            <button @click="navigate('hr/notifications')" class="view-all-btn">View All</button>
          </div>
            ...
            <div v-if="recentApplications.length > 0">
              <div v-for="application in recentApplications" :key="application.id" class="application-item">
                <div class="applicant-avatar">
                  <img v-if="application.avatar" :src="application.avatar" :alt="application.name" class="avatar-img">
                ...
                <div class="applicant-info">
                  <h4 class="applicant-name">{{ application.candidate_name }}</h4>
                  <p class="applicant-email">{{ application.candidate_email }}</p>
                  <p class="job-title">{{ application.application_data.job_title }}</p>
                </div>

                <div class="application-actions">
                  <button class="btn-review" @click="routeToScreeningPage(application.application_data.job_id)"
          ...
          <div class="interviews-list" v-if="upcomingInterviews.length > 0">
            <div v-for="interview in upcomingInterviews" :key="interview.id" class="interview-item">
              ...
              <div class="interview-actions">
                <button class="btn-join" @click="joinInterview(interview)" :disabled="!interview.meeting_link"
                  :title="interview.meeting_link ? 'Join Meeting' : 'No meeting link'">
                  <i class="fas fa-video"></i>
                </button>
                <button
                  v-if="interview.interview_type === 'final_interview' || interview.interview_type === 'behavioral_interview'"
                  class="btn-evaluate" @click="openEvaluationModal(interview)"
                  :title="interview.scorecard ? 'Edit Evaluation' : 'Evaluate Interview'">
                  <i class="fas fa-star"></i>
                  {{ interview.scorecard ? 'Edit' : 'Evaluate' }}
                </button>
    ...
    <div v-if="showEvaluationModal" class="modal-overlay" @click.self="closeEvaluationModal">
      <div class="modal-container evaluation-modal">
        <div class="modal-header">
          <h3>
            <i class="fas fa-star"></i>
            {{ selectedInterview?.scorecard ? 'Edit' : 'Submit' }} Interview Evaluation
          </h3>
        ...
        <div class="modal-body">
          <div class="interview-details-summary">
            ...
            <p>{{ selectedInterview?.position }} - {{ formatInterviewType(selectedInterview?.interview_type) }}</p>
            <p class="interview-time">{{ formatDate(selectedInterview?.scheduled_time) }} at {{
              formatTime(selectedInterview?.scheduled_time) }}</p>
          </div>

          <form @submit.prevent="submitEvaluation">
            <div class="evaluation-section">
              <h4>Interview Scores (1-5 scale)</h4>

              <div class="form-row">
                <div class="form-group">
                  <label for="technical_score">Technical Skills</label>
                  <input id="technical_score" v-model.number="evaluationForm.technical_score" type="number" min="0"
                    max="5" step="0.1" required />
                </div>

                <div class="form-group">
                  <label for="communication_score">Communication</label>
                  <input id="communication_score" v-model.number="evaluationForm.communication_score" type="number"
              ...
              <div class="form-row">
                <div class="form-group">
                  ...
                  <input id="problem_solving_score" v-model.number="evaluationForm.problem_solving_score" type="number"
                    min="0" max="5" step="0.1" required />
                </div>

                <div class="form-group">
                  <label for="cultural_fit_score">Cultural Fit</label>
                  <input id="cultural_fit_score" v-model.number="evaluationForm.cultural_fit_score" type="number"
                    min="0" max="5" step="0.1" required />
                </div>
              </div>
            </div>
            ...
            <div class="evaluation-section">
              ...
              <div class="form-group">
                ...
                <textarea id="strengths" v-model="evaluationForm.strengths" rows="3"
                  placeholder="Key strengths observed during the interview..."></textarea>
              </div>

              <div class="form-group">
                <label for="weaknesses">Areas for Improvement</label>
                <textarea id="weaknesses" v-model="evaluationForm.weaknesses" rows="3"
                  placeholder="Areas where candidate could improve..."></textarea>
              </div>

              <div class="form-group">
                <label for="feedback_notes">Additional Notes</label>
                <textarea id="feedback_notes" v-model="evaluationForm.feedback_notes" rows="4"
                  placeholder="Any other observations or comments..."></textarea>
              </div>

              <div class="form-group">
                <label for="recommendation">Recommendation</label>
                <select id="recommendation" v-model="evaluationForm.recommendation" required>
            ...
            <div class="modal-footer">
              <button type="button" @click="closeEvaluationModal" class="btn-cancel" :disabled="submittingEvaluation">
                Cancel
              </button>
              <button type="submit" class="btn-submit" :disabled="submittingEvaluation">
                <i v-if="submittingEvaluation" class="fas fa-spinner fa-spin"></i>
...
const routeToScreeningPage = (jobId) => {
  router.push({ path: '/hr/screening', query: { job_id: jobId } })
}
...
async function loadCandidatesForJob() {
  ...
  try {
    loading.value = true
    console.log('Loading candidate applications')
    ...
    console.log("Load Candidates For Job:", response.data.applications)
    if (response.data && response.data.applications) {
      ...
      console.log('candidates info:', candidates.value)
    } else {
      candidates.value = []
    }
  } catch (error) {
...
async function loadNotifications() {
  ...
  try {
    ...
    notifications.value = response.data
      .filter(n => n.recipient_id === userResponse.data.id)
...
function getNotificationIconClass(message) {
  if (!message) return 'notification-icon-info'
  const msg = message.toLowerCase()
  if (msg.includes('hired') || msg.includes('congratulations')) return 'notification-icon-success'
  if (msg.includes('offer')) return 'notification-icon-offer'
  if (msg.includes('interview')) return 'notification-icon-interview'
  if (msg.includes('technical test') || msg.includes('test')) return 'notification-icon-test'
...
function openEvaluationModal(interview) {
  ...
  // If scorecard exists, pre-fill the form
  if (interview.scorecard) {
    evaluationForm.value = {
      ...
      cultural_fit_score: interview.scorecard.cultural_fit_score || 0,
...
async function submitEvaluation() {
  try {
    ...
    await axios.post('/api/pipeline/scorecard', payload, {
      headers: { Authorization: `Bearer ${getAuthToken()}` }
    })
...
.interview-details-summary h4 {
  margin: 0 0 5px 0;
  color: #2c3e50;
  font-size: 1.1rem;
}

.interview-details-summary p {
  margin: 3px 0;
  color: #7f8c8d;
  font-size: 0.9rem;
}

.interview-details-summary .interview-time {
  color: #3498db;
  font-weight: 600;
}
